...
import type { AllyType } from '../../data/mockAllies';

const typeOptions: { value: AllyType; label: string; color: string }[] = [
  { value: 'veterinary', label: 'Veterinarias', color: '#1C4E80' },
  { value: 'shelter', label: 'Refugios', color: '#6B2C91' },
  { value: 'ngo', label: 'Asociaciones', color: '#0E7490' },
  { value: 'educational', label: 'Educativos', color: '#15803D' },
];

type AllyFiltersProps = {
  activeTypes: AllyType[];
  onChange: (types: AllyType[]) => void;
};

export function AllyFilters({ activeTypes, onChange }: AllyFiltersProps) {
  const [open, setOpen] = useState(false);

  const toggle = (type: AllyType) => {
    onChange(activeTypes.includes(type) ? activeTypes.filter((t) => t !== type) : [...activeTypes, type]);
  };
          ...
          >
            <div className="p-4">
              ...
              <p className="mt-3 text-[11px] text-neutral-400">
                Sin seleccionar, se muestran todos los aliados.
              </p>
            </div>
          </motion.div>
        )}
